...
function persistOneBlock(asset, blockTxData, areturn) {
    if (!blockTxData) {
    ...
    DBWallet.transaction(function (t) {
        ...
        Logger.info('Success persist Block');
    }).catch(e => {
        console.log('Failed persist Block')
    })
}
...
getOneBlock4DB('BTC', 160001, (err, res) => {
    console.log(err);
    console.log(res);
    persistOneBlock('BTC', res,function (err,res) {
        if(err){
            console.log(err);
            return;
        }
